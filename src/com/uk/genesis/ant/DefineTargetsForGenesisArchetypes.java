...
public class DefineTargetsForGenesisArchetypes extends BaseGenesisTask {
    ...
    public void setTargetsdependon(final String depends) {
    ...
    public void execute() {
        ...
        try {
            ...
            log("Failed to generate targets for actions: " + ex.getMessage(),
                    ex, Project.MSG_ERR);
     ...
     */
    private void defineTargetsForAction(final GenesisArchetype archetype,
        ...
        log("Defined target " + target.getName() + " based on Genesis Action",
                Project.MSG_VERBOSE);
    }

    private GenesisObjectReference getBestReferenceForRequirement(
            final GenesisArchetypeRequirement req) {
        ...
        while (curType != null) {
            for (GenesisObjectReference ref : availableObjects) {
                if (ref.getType().equals(curType.getQualifiedName())) {
                    // We've found a declared object that matches the required
                    // type
                    return ref;
                }
            }
    ...
    private GenesisObject[] enumerateObjectsForRequirement(
        ...
        if (bestReference == null) {
            ...
            return EnumUtils.enumerateChildObjectsOfType(bestRefObj,
                    req.getType());
        }
    }
    ...
    public class MultiRequirementTaskExecutor extends Task {
        ...
        public MultiRequirementTaskExecutor(final GenesisArchetype action,
                final GenesisArchetypeOperation operation,
                final List<FulfilledGenesisActionRequirement> reqs) {
        ...
        private void executeRequirementAxis(final int pos) {
            ...
            if (reqs.size() > pos) {
                ...
                for (int i = 0; i < reqs.size(); ++ i) {
                    ...
                    // Log detail
                    log("  " + axisInstances[i].getQualifiedName() + " ("
                            + axisInstances[i].getType().getQualifiedName()
                            + ")", Project.MSG_INFO);
                }
