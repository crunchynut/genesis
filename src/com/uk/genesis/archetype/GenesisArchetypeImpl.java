...
public class GenesisArchetypeImpl implements GenesisArchetype {
    ...
    public GenesisArchetypeImpl(final File root,
            final ModelReader modelReader, final Element actionEl)
    ...
    public File getBuildFile() {
        return new File(root, actionEl.getAttribute("buildfile"))
                .getAbsoluteFile();
    }
    ...
    private GenesisArchetypeRequirement[] parseRequirements(
        ...
        try {
            ...
            NodeList nodes = (NodeList) expr.evaluate(actionEl,
            ...
            for (int i = 0; i < nodes.getLength(); ++ i) {
                ...
                GenesisObjectType type = modelReader
                        .findSingleObjectType(typeName);
                String qnameProperty = el.getAttribute("qnameproperty");
                String nameProperty = el.getAttribute("nameproperty");

                if (type == null) {
                    throw new InvalidGenesisConfigurationException("Type "
                            + typeName
                            + " was not valid in declaration for action "
                            + getName());
                }

                GenesisArchetypeRequirement requirement = new GenesisArchetypeRequirement(
                        type, qnameProperty, nameProperty);

                result.add(requirement);
            }
            return result
                    .toArray(new GenesisArchetypeRequirement[result.size()]);
        } catch (XPathExpressionException ex) {
            throw new InvalidGenesisConfigurationException(
                    "Failed to compile xpath", ex);
        } catch (ModelException ex) {
            throw new InvalidGenesisConfigurationException(
                    "Failed to match requirement type", ex);
        }
    }
    ...
    private GenesisArchetypeOperation[] parseOperations()
        ...
        try {
            ...
            for (int i = 0; i < nodes.getLength(); ++ i) {
                ...
                GenesisArchetypeOperation op = new GenesisArchetypeOperation(
                        name, target);
    ...
    private XPathExpression getXPath(final String expr)
