...
public class ArchetypeFactoryImpl implements ArchetypeFactory {
    ...
    /**
     * Creates a new ArchetypeFactory implementation
     * using the given environment.
     * @param env the environment.
     * @param modelReader the model reader to use.
     * @throws InvalidGenesisConfigurationException.
     */
    public ArchetypeFactoryImpl(final GenesisEnvironment env,
            final ModelReader modelReader)
            throws InvalidGenesisConfigurationException {
        // If there was no archetype declaration,
        // then just store an empty list of actions
        ...
        // Parse the actions
        parseActions(env.getGenesisRoot(), modelReader,
                env.getGenesisArchetypes());
    }
     ...
     * @param root the root.
     * @param modelReader the model reader to use
     *  to resolve any requirement types.
     * @param actionsContainer the container for the actions.
     * @throws InvalidGenesisConfigurationException.
     */
    private void parseActions(final File root,
                                final ModelReader modelReader,
        ...
        try {
            ...
            NodeList nodes = (NodeList) expr.evaluate(actionsContainer,
            ...
            throw new InvalidGenesisConfigurationException(
                    "Failed to compile xpath", ex);
        }
    }

    /**
     * Builds an action instance from the given xml element.
     * @param root the file system root that any paths
     * specified in the action should start from.
     * @param modelReader the model reader to use to
     * resolve any requirement types.
     * @param action definition of the action.
     * @return the action.
     */
    private GenesisArchetype buildAction(final File root,
            final ModelReader modelReader, final Element action)
            throws InvalidGenesisConfigurationException {
