...
/**
 * Generates 3DES passwords. Encrypts plaintext passwords
 * in Archetype source. Requires encryptionservice parameter
 * specifying the WebLogic "../domain/security" directory
...
public class TripleDes extends Task {
    ...
    public void setEncryptionservice(final String encryptionservice) {
        ...
        for (Iterator<PropertyMatch> pm = matches.iterator(); pm.hasNext();) {
            ...
            for (Iterator<?> it = map.keySet().iterator(); it.hasNext();) {
                ...
                if (globalPattern.matcher(key.toString()).matches()) {
                    fs.addFilter((key.toString() + "Encrypted"),
                            ces.encrypt(value.toString()));
                }
            }
        }
    }
